#!/usr/bin/env python3
"""
Oracle Cloud Object Storage — Long-Running TCP Latency Monitor

Continuously probes all 42 OCI endpoints in cycles (30 probes each).
Press Ctrl+C to stop at any time — accumulated data is saved to JSON,
then Excel + HTML reports are generated from all cycles.

Usage:
    python oci_latency_monitor.py

Outputs:
    - oci_monitor_data.json        (raw data, saved after each cycle)
    - oci_monitor_report.xlsx      (Excel workbook)
    - oci_monitor_chart.html       (ECharts time-series chart)
"""
...
def load_data(path):
    if path.exists():
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"cycles": [], "started_at": None, "endpoints": []}
...
def _loss_fill(lost, st):
    if lost == 0: return st["green"]
    if lost < 15: return st["yellow"]
    return st["red"]

def build_excel(data, output_path):
    """Build Excel from multi-cycle data."""
    wb = Workbook(); st = _styles()

    # --- Sheet 1: Cycle Summary ---
    ws = wb.active; ws.title = "Cycle Summary"
    cycles = data["cycles"]
    ws.merge_cells("A1:G1")
    ws["A1"].value = f"OCI Long-Running Monitor — {len(cycles)} cycles, {PROBE_COUNT} probes/cycle"
    ws["A1"].font = st["tfont"]
    ws["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 32

    headers = ["Cycle", "Timestamp", "Region Code", "Region Name (CN)", "Min (ms)", "Avg (ms)", "Max (ms)"]
    widths = [8, 22, 16, 24, 12, 12, 12]
    ...
    for ci, cycle in enumerate(cycles, 1):
        ...
        for ep in cycle.get("endpoints", []):
            lats = ep.get("latencies", [])
            if lats:
                vals = [ci, ts, ep["region"], ep["name_cn"], round(min(lats), 2), round(statistics.mean(lats), 2), round(max(lats), 2)]
            else:
                vals = [ci, ts, ep["region"], ep["name_cn"], "FAIL", "FAIL", "FAIL"]
    ...
    # --- Sheet 2: Per-Endpoint Averages ---
    ws2 = wb.create_sheet("Endpoint Averages")
    ws2.merge_cells("A1:L1")
    ws2["A1"].value = f"Per-Endpoint Averages Across {len(cycles)} Cycles"
    ws2["A1"].font = st["tfont"]
    ws2["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws2.row_dimensions[1].height = 32

    h2 = ["Region Code", "Region Name (CN)", "Region Name (EN)", "Hostname",
          "Cycles", "Success Rate", "Lost Avg", "Min (ms)", "Max (ms)", "Avg (ms)", "Trimmed Avg (ms)", "StdDev (ms)"]
    w2 = [16, 22, 26, 46, 8, 12, 10, 12, 12, 12, 16, 14]
    ...
    endpoint_data = {}
    for ep_def in data.get("endpoints", []):
        key = ep_def["region"]
        endpoint_data[key] = {"name_cn": ep_def["name_cn"], "name_en": ep_def["name_en"], "host": ep_def["host"], "all_lats": []}

    for cycle in cycles:
        for ep in cycle.get("endpoints", []):
            key = ep["region"]
            if key in endpoint_data:
                endpoint_data[key]["all_lats"].extend(ep.get("latencies", []))

    row = 3
    for region, info in endpoint_data.items():
        all_lats = info["all_lats"]
        total_probes = len(cycles) * PROBE_COUNT
        success = len(all_lats)
        lost_avg = round((total_probes - success) / len(cycles), 1) if cycles else 0
        if success > 0:
            mn, mx = round(min(all_lats), 2), round(max(all_lats), 2)
            avg = round(statistics.mean(all_lats), 2)
            ta = trimmed_mean(all_lats)
            sd = round(statistics.stdev(all_lats), 2) if len(all_lats) >= 2 else 0
        else:
            mn = mx = avg = ta = sd = None
        vals = [region, info["name_cn"], info["name_en"], info["host"],
                len(cycles), round(success / total_probes * 100, 1) if total_probes > 0 else 0,
                lost_avg, mn, mx, avg, ta, sd]
        for c, val in enumerate(vals, 1):
            cell = ws2.cell(row=row, column=c, value=val if val is not None else "N/A")
            cell.border = st["border"]
            cell.alignment = st["lalign"] if c <= 4 else st["calign"]
        _set_fill(ws2.cell(row=row, column=10), _latency_fill(avg, st))
        _set_fill(ws2.cell(row=row, column=11), _latency_fill(ta, st))
        row += 1

    ws2.freeze_panes = "A3"
    ws2.auto_filter.ref = f"A2:L{row - 1}"

    wb.save(output_path)
    return output_path

# ===========================================================================
#  ECHARTS HTML (time-series)
# ===========================================================================

def build_chart_html(data, output_path):
    """Generate time-series chart: X=cycle timestamp, Y=avg latency per endpoint."""

    cycles = data["cycles"]
    if not cycles:
        output_path.write_text("<html><body><h1>No data collected</h1></body></html>", encoding="utf-8")
        return output_path

    # Build time-series: for each endpoint, track avg latency per cycle
    chart_data = []
    for sheet_name, prefixes, colors in REGION_GROUPS:
        eps = []
        # Find matching endpoints from the first cycle
        ep_map = {}
        if cycles:
            for ep in cycles[0].get("endpoints", []):
                if ep["region"].split("-")[0] in prefixes:
                    ep_map[ep["region"]] = {"name_cn": ep["name_cn"], "name_en": ep["name_en"]}

        for i, (region_code, info) in enumerate(ep_map.items()):
            # Collect avg per cycle
            avgs = []
            timestamps = []
            for ci, cycle in enumerate(cycles):
                ts = cycle.get("timestamp", f"Cycle {ci + 1}")
                timestamps.append(ts)
                for ep in cycle.get("endpoints", []):
                    if ep["region"] == region_code:
                        lats = ep.get("latencies", [])
                        avgs.append(round(statistics.mean(lats), 2) if lats else None)
                        break
                else:
                    avgs.append(None)
            eps.append({
                "id": region_code,
                "label": info["name_en"],
                "label_cn": info["name_cn"],
                "color": colors[i % len(colors)],
                "timestamps": timestamps,
                "avgs": avgs,
                "overall_avg": round(statistics.mean([a for a in avgs if a is not None]), 2) if any(a is not None for a in avgs) else None,
            })
...
<title>OCI Long-Running Monitor — ECharts</title>
...
.header{{background:linear-gradient(135deg,#0f0f23,#1a1a3e);padding:12px 24px;border-bottom:1px solid #2a2a4a;display:flex;align-items:center;justify-content:space-between}}
...
.ep-row{{display:flex;align-items:center;padding:4px 14px 4px 44px;font-size:12px;color:#aaa;cursor:pointer}}
...
<div class="header">
  <div>
    <h1>OCI Long-Running Latency Monitor</h1>
    <div class="sub">42 endpoints · {len(cycles)} cycles · {PROBE_COUNT} probes/cycle · X-axis = cycle timestamp</div>
  </div>
</div>
<div class="main">
  ...
  <div class="chart-area">
    <div class="toolbar">
      ...
      <button onclick="toggleClip()" id="btn-clip">Clip Extremes</button>
  ...
  div.innerHTML = `
    ...
    <div class="ep-list open" id="list_${{ri}}">
      ${{region.endpoints.map((ep, ei) => {{
        seriesMeta.push({{id:ep.id, regionIdx:ri, epIdx:ei, avg:ep.overall_avg}});
        return `
          <div class="ep-row">
...
const option = {{
  ...
  grid: {{ left: 65, right: 30, top: 20, bottom: 80 }},
  xAxis: {{
    type: 'category',
    data: DATA[0].endpoints[0].timestamps,
    name: 'Cycle',
  ...
  dataZoom: [
    {{ type: 'slider', start: 0, end: 100, height: 24, bottom: 8,
       borderColor: '#333', backgroundColor: '#1a1a2e',
       dataBackground: {{ lineStyle: {{ color: '#5470C6', opacity: 0.4 }}, areaStyle: {{ color: '#5470C6', opacity: 0.1 }} }},
       selectedDataBackground: {{ lineStyle: {{ color: '#5470C6' }}, areaStyle: {{ color: '#5470C6', opacity: 0.2 }} }},
       handleStyle: {{ color: '#5470C6', borderColor: '#5470C6' }},
       textStyle: {{ color: '#888', fontSize: 10 }} }},
    {{ type: 'inside', start: 0, end: 100 }}
  ],
  toolbox: {{ right: 30, top: 5, itemSize: 14,
    feature: {{ saveAsImage: {{ title: 'Save', backgroundColor: '#1a1a2e' }},
...
function updateStats() {{
  ...
  const visible = seriesMeta.filter(m => {{
    ...
    return sel[ep.label + ' (' + ep.label_cn + ')'] !== false;
  }});
  if (visible.length === 0) return;
  const avgs = visible.map(m => m.avg).filter(a => a != null).sort((a,b)=>a-b);
  if (avgs.length === 0) return;
}}

function toggleRegion(ri) {{
  document.getElementById('list_' + ri).classList.toggle('open');
  document.getElementById('arr_' + ri).classList.toggle('open');
}}

function toggleRegionSeries(ri, visible) {{
  const names = DATA[ri].endpoints.map(ep => ep.label + ' (' + ep.label_cn + ')');
  const sel = {{}}; names.forEach(n => sel[n] = visible);
...
function toggleEp(id) {{
  ...
  const name = ep.label + ' (' + ep.label_cn + ')';
  const cur = chart.getOption().legend[0].selected;
  const nv = cur[name] === false ? true : false;
  chart.setOption({{ legend: {{ selected: {{ [name]: nv }} }} }});
...
function selAll() {{
  const sel = {{}}; legendData.forEach(n => sel[n] = true);
  chart.setOption({{ legend: {{ selected: sel }} }});
...
function selNone() {{
  const sel = {{}}; legendData.forEach(n => sel[n] = false);
  chart.setOption({{ legend: {{ selected: sel }} }});
...
function applySelection(selected) {{
  ...
  legendData.forEach(name => {{
    const meta = seriesMeta.find(m => (DATA[m.regionIdx].endpoints[m.epIdx].label + ' (' + DATA[m.regionIdx].endpoints[m.epIdx].label_cn + ')') === name);
...
function toggleClip() {{
  ...
  document.getElementById('btn-clip').classList.toggle('on', clipEnabled);
  if (!clipEnabled) {{ chart.setOption({{ yAxis: {{ max: null }} }}); return; }}
  const sel = chart.getOption().legend[0].selected;
  const allVals = [];
  series.forEach((s, i) => {{
    if (sel[legendData[i]] !== false && s.data) s.data.forEach(v => {{ if (v != null) allVals.push(v); }});
  }});
  if (allVals.length === 0) return;
  allVals.sort((a,b) => a - b);
  chart.setOption({{ yAxis: {{ max: Math.max(allVals[Math.floor(allVals.length * 0.95)], 100) }} }});
}}
  ...
  setTimeout(() => {{
    ...
    document.querySelectorAll('.ep-row input[type=checkbox]').forEach(cb => {{
      ...
      if (spans.length >= 2) {{
        ...
        const fullName = spans[1].textContent + ' (' + (cn ? cn.textContent : '') + ')';
...
def main():
    data_path = Path.cwd() / "oci_monitor_data.json"
    data = load_data(data_path)

    if not data.get("endpoints"):
        data["endpoints"] = [{"region": r, "name_cn": c, "name_en": e, "host": h} for r, c, e, h in ENDPOINTS]

    if not data.get("started_at"):
        data["started_at"] = datetime.now(timezone.utc).isoformat()

    total = len(ENDPOINTS)
    cycle_num = len(data["cycles"]) + 1
    stopped = False
    ...
    if sys.platform == "win32":
        signal.signal(signal.SIGBREAK, on_stop)

    print(f"OCI Long-Running Latency Monitor")
    print(f"  Endpoints: {total}  |  Probes per cycle: {PROBE_COUNT}  |  Port: {PORT}")
    print(f"  Data file: {data_path}")
    print(f"  Press Ctrl+C to stop and generate reports\n")

    if data["cycles"]:
        print(f"Resuming from {len(data['cycles'])} previous cycles\n")

    try:
        while not stopped:
            print(f"=== Cycle {cycle_num}  [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ===")

            cycle = {"cycle": cycle_num, "timestamp": datetime.now(timezone.utc).isoformat(), "endpoints": []}

            for idx, (region, name_cn, name_en, host) in enumerate(ENDPOINTS, 1):
                if stopped:
                    break
                print(f"  [{idx:2d}/{total}] {name_en} ({region}) ...", end=" ", flush=True)
                latencies = probe_host(host, count=PROBE_COUNT)
                received = len(latencies)
                if received == 0:
                    print("FAIL")
                else:
                    print(f"min={min(latencies):.1f}  avg={statistics.mean(latencies):.1f}  max={max(latencies):.1f}")
                cycle["endpoints"].append({
            ...
            if cycle_num % 5 == 0:
                ...
                print(f"  [Data saved: {len(data['cycles'])} cycles]\n")
            else:
                print()
        ...
        if data["cycles"]:
            save_data(data_path, data)
            print(f"\nData saved: {len(data['cycles'])} cycles → {data_path}")
        ...
        # Generate Excel
        xlsx_path = Path.cwd() / "oci_monitor_report.xlsx"
        build_excel(data, xlsx_path)
        print(f"Excel report: {xlsx_path}")

        # Generate HTML
        html_path = Path.cwd() / "oci_monitor_chart.html"
        build_chart_html(data, html_path)
        print(f"HTML chart:  {html_path}")

        started = data.get("started_at", "?")
        print(f"\nDone. Monitored from {started} across {len(data['cycles'])} cycles.")
